...
<div class="zaixiankf">
    ...
    <div class="second" v-show="step1==1">
        <div class="yangshi"><label>类型:</label>
            <p><span :class="{choose:chooseidx==0}" v-for="(item,index) in yaotype" :key="index" @touchstart="changeType(index,item.label)">{{item.label}}</span></p>
        </div>
        <div class="yangshi"><label>品牌:</label>
            <p><span @touchstart="changePinpai(index,item.id,item.lable)" v-for="(item,index) in medicineManner" :key="index" :class="{choose:pinpaiIdx==index}">{{item.lable}}</span></p>
        </div>
        <div class="yangshi"><label>药态:</label>
            <p><span @touchstart="changeYaotai(index,item.shapeId,item.shapeName)" v-for="(item,index) in yaotaiList" :key="index" :class="{choose:yaotaiIdx==index}">{{item.shapeName}}</span></p>
        </div>
        <div class="yaofang">
            <span>药方:</span>
            <div class="list">
                <p v-for="(item,index) in arr" :key="index"><label>{{item.medicineName}}</label><input type="number" readonly v-model='item.howWeight' @input="lengthNum(index)" @touchstart="hideYaocai(index)"><em>g</em>
                ...
                <div class="addMoban" @touchstart="showMedi">添加</div>
                <div class="addMoban" @touchstart="findModelByShape">使用模版/历史</div>
                <div class="jiliang">
                    剂量:共<input type="number" v-model="preNum" :placeholder="preNum1" @focus="qingchu1" @blur="huifu1" @input="shuru1">剂,每日<input type="number" :placeholder="preOnce1" v-model="preOnce" @focus="qingchu2" @blur="huifu2" @input="shuru2">剂,1剂分<input type="number" :placeholder="dayNum1" v-model="dayNum" @focus="qingchu3" @blur="huifu3" @input="shuru3">次服用
                </div>
            </div>
        </div>
            ...
            <div class='keyboard02' :style='{display:k2}'>
                <div class='left02 number'>
                    ...
                    <div class='hang'>
                        ...
                        <div data-key="hs" hover-class='active' hover-stay-time='100' @touchstart='delkey' style='width:60px'><img src='../assets/img/hs.png' style='width:35px;height:35px;margin-top:8px;'></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="third" v-show="step1==2">
        <div class="feiyong">
            <span>费用:{{total.toFixed(2)}}元(药费)+{{zhenjinte}}元(诊金)</span><label>合计:{{total1.toFixed(2)}}元</label>
        </div>
        <div class="fenlei">
            <div class="qianF">
                <label>诊金:</label>
                <p><span @touchstart="showJin">{{zhenjinte}}元</span><label v-show="chooseZhen"><em v-for="(item,index) in kekzhenjinana" :key="index" @touchstart="choosePrice(item.price)">{{item.price}}</em></label></p>
            </div>
            <div class="qianF">
                <label>购药前查看药方:</label>
                <p><span v-for="(item,index) in chakan" :key="index" @touchstart="changeChakan(index,item.name)" :class="{choose:chakanIdx==index}">{{item.name}}</span></p>
            </div>
            <div class="jinji">
                <p><label>服药禁忌:</label><em @touchstart="hideList">({{textJinji}})</em></p>
                <div class="list" v-show="hidejinji">
                    <span v-for='(item,index) in list' :key="index" :class="{chooseJin:selectIndex[index].sureid}" @touchstart="xuanJinji(index,item.tabooName,item.tabooId)">{{item.tabooName}}</span>
                </div>
            </div>
            <div class="buchong">
                <span>补充说明:</span>
    ...
    <div class="four" v-show="step1==3">
        ...
        <div class="info">
            ...
            <p class="fuyaojinji"><span>服药禁忌:</span><label v-for="(item,index) in json" :key="index">{{item.tabooName}}、</label><label v-if="json.length==0">无</label></p>
            <p><span>补充说明:</span><label>{{preEnjoin}}</label></p>
        </div>
        <div class="xian" style="clear:both;"></div>
    </div>
    <div class="nextStep">
        <div v-if="step1==0" @touchstart="backHome">返回上一页</div>
        <div v-if="step1!=0" @touchstart="backstep">上一步</div>
        <div @touchstart="stepGo" v-if="step1!=3">下一步</div>
        <div v-if="step1==3" @touchstart="linePrescription">确认开方</div>
    </div>
    <div class="moban" v-if="showMoban">
        <div class="quxiaoMo" @touchstart="quxiaoMo">取消</div>
        <div class="detail">
            <div v-for="(item,index) in moban" :key="index" @touchstart="chooseMo(item.list)">
