	...
	<div class="mobanList">
        ...
        <div class="detail">
            <div v-for="(item,index) in moban" :key="index">
                <div><span>名称:</span>{{item.modelName}}　<p><em @click="deleteM(item.modelId)">删除</em><em @click="xiugaiM(item.modelId)">修改</em></p></div>
                <div>
                    <span>内容:</span>
...
<script>
	import {findModelByShape,deleteModel} from "api/doctor"
	...
	export default {
		...
		methods:{
            ...
            goAddm(){
                this.$router.push({path:'/addMoban',query:{shapeId:this.$route.query.shapeId,brandId:this.$route.query.brandId}})
            },
            xiugaiM(modelId){
                this.$router.push({path:'/addMoban',query:{modelId:modelId,shapeId:this.$route.query.shapeId,brandId:this.$route.query.brandId}})
            },
			findModelByShape(){
                let list = {
                    token:this.token,
