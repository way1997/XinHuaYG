...
<div class="indexPage">
    <div class="head">
        <div>
            <input type="text" placeholder="输入医生或疾病" v-model="doctorTel" /><em class="search" @click="searchYs">搜索</em>
        </div>
    </div>
    <div class="xiaoxi">
    ...
    <div class="tuijian">
        ...
        <div v-for="(item, index) in tuijianYs" :key="index">
            <div @click="goDetail(item.doctorId)" style="width: 100%">
                ...
                <div>
                    <span v-for="(item, index) in item.list_adept" :key="index">{{
    ...
    <div class="tabList">
        ...
        <div class="detailList">
            <div v-for="(item, index) in quanbuWenzhang" :key="index" @click="gowenxq(item.id, item)">
                ...
                <div>{{ item.titles }}</div>
                <p>
                    来自：{{ item.sources }} {{ item.departmentName
            }}{{ item.doctorLevelName }}
                </p>
                <p>浏览量：123</p>
            </div>
        </div>
...
export default {
    ...
    created() {
        ...
        window.removeEventListener("popstate", that.unloadHander, false);
        // 添加事件
        window.addEventListener("popstate", that.unloadHander, false);
        window.addEventListener('beforeunload', that.unloadHander, false)
        window.addEventListener('unload', that.unloadHander, false)
        console.log(window.addEventListener)
    },
    computed: {},
    montued() {
    ...
    destroyed() {
        // // 移除事件
        //alert(window.removeEventListener)
        window.removeEventListener("popstate", this.unloadHander);
    ...
    methods: {
        beforeunloadHandler() {
            alert('beforeunloadHandler被触发')

        },
        unloadHandler(e) {

            alert('unloadHandler' + e)
            //判断是窗口关闭还是刷新
            // if (this._gap_time <= 5) {
            //     //如果是登录状态，关闭窗口前，移除用户
