<template>
	<div class="zhaoYs">
        ...
        <div class="search"><input type="text" placeholder="输入医生名字" v-model="doctorTel" style="border:1px solid #e9e9e9;"><em class="searchEm" @click="searchYs">搜索</em></div>
        <div class="xian"></div>
        <div class="list">
            <div class="zhonglei">
                <p v-for="(item,index) in kemu" :key="index" :class="{choose:chooIdx==index}" @click="change(index,item.departmentId)"><span>{{item.departmentName}}</span></p>
            </div>
            <div class="yisheng">
...
<script>
	import {sectionList,getDoctorByTel,relation} from "api/patient"
