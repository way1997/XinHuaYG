<template>
	...
	<div class="backHome"  @click="goBack">返回上一级</div>
        ...
        <div class="dateTime" v-show="pickerVisible">
            ...
            :startDate="startDate"
            @cancel="hideDate"
            @confirm="handleConfirm" style="display:block;">
            </mt-datetime-picker>
        </div>
	</div>
</template>
<script>
	import {insertArchives,selectById} from 'api/patient'
	...
	export default {
		...
		data (){
			return {
                ...
                tiaozhuanType0001: 0,
                guanxi:'选择成员关系',
                array: [{values:['选择成员关系','自 己', '父 母', '子 女', '爱 人','其 他']}],
                chooseR:false,
                index: 0,
                sexs: [{
                sex: '男'
      ...
      goGai(num){
        if(num==1){
          this.$router.push({path:'/jibingAdd',query:{num:num,id:this.chengyuanId}})
        }else if(num==2){
          this.$router.push({path:'/jiwangbingshi',query:{num:num,id:this.chengyuanId}})
        }else if(num==3){
          this.$router.push({path:'/jiazubingshi',query:{num:num,id:this.chengyuanId}})
        }else if(num==4){
          this.$router.push({path:'/guominshi',query:{num:num,id:this.chengyuanId}})
        }
