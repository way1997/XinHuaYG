<template>
	<div class="yishengxq">
        ...
        <div class="tiaoli">
            <h1>擅长调理</h1>
            <div><span v-for="(item,index) in list3" :key="index">{{item.medicineSubjectName}}</span></div>
        </div>
        <div class="jianjie">
            <h1>医生简介</h1>
        ...
        <div class="zixun" @click="goLiaotian">
            ...
            <!-- <div style="width:2.5rem;height:0.9rem;text-align:center;line-height:0.9rem;font-size:0.36rem;margin:0 auto;">
                ...
                <script type="text/wxtag-template">
                    <style>.btn { font-size:19px;border:none;background:none;color:#fff;line-height:51px;}</style>
...
<script>
	...
	export default {
		...
		methods:{
            ...
            godaxie(){
                this.$router.push({path:'/daxieYs',query:{doctorId:this.doctorId,doctorPhoto:this.lists.doctorPhoto,doctorName:this.lists.doctorName}})
            },
            goLiaotian(){
                if(this.isRelation == 1){
                    this.$router.push({path:'/wenzhenZx',query:{doctorId:this.doctorId,doctorPhoto:this.lists.doctorPhoto,doctorName:this.lists.doctorName,patientIds:''}})
                }else{
        ...
                    token:this.token,
                ...
                relation(list).then((res)=>{
                    if(res.type == true){
                        this.$toast('建立成功');
                        this.$refs.confirm.hide();
                        this.$router.push({path:'/wenzhenZx',query:{doctorId:this.doctorId,doctorPhoto:this.lists.doctorPhoto,doctorName:this.lists.doctorName}})  
                    }
                })
            },
			doctorDetails(){
