    ...
    <van-form>
        ...
        <van-popup v-model="ksShow" position="bottom" closeable close-icon-position="top-right" :style="{ height: '80%' }">
            <div class="popup ks-popup">
                ...
                <van-row>
                    <scroll-view class='xuanzekeshi' catchtouchmove='true' scroll-y="true" style="  display: flex">
                        <view wx:for="{{shanchangt}}" class='zhuanghuanyixia'>
                            <view class='text_wrap7' bindtap='shanchang_color' id="{{item.departmentId}}" style='{{item.departmentName==keshitext?"color:#fff;border:none;background:#00b0c2":"color:#222"}}' data-text="{{item.departmentName}}">{{item.departmentName}}</view>
                        </view>
                    </scroll-view>
                </van-row>
            </div>
