<template>
	<div class="paizhaokf">
        ...
        <div class="second">
            <h1>药方信息</h1>
           <div class="yangshi"><label>类型:</label><p><span :class="{choose:chooseidx==0}" v-for="(item,index) in yaotype" :key="index" @click="changeType(index,item.label)">{{item.label}}</span></p></div>
           <div class="yangshi"><label>品牌:</label><p><span @click="changePinpai(index,item.id,item.lable)" v-for="(item,index) in medicineManner" :key="index" :class="{choose:pinpaiIdx==index}">{{item.lable}}</span></p></div>
           <div class="yangshi"><label>药态:</label><p><span  @click="changeYaotai(index,item.shapeId,item.shapeName)" v-for="(item,index) in yaotaiList" :key="index" :class="{choose:yaotaiIdx==index}">{{item.shapeName}}</span></p></div>
...
<script>
	import {taboo,priceList,prescriptionType,modelType,findBrandList,saveOnlinePrescribing,doctorSelAllDrug} from "api/doctor"
            ...
            taboo(){//禁忌
                ...
                taboo(list).then((res)=>{
                  ...
                  res.data.forEach(function(item, index) {
                      that.selectIndex.push({sureid:0});
                  })
