    ...
    <van-form>
        ...
        <van-popup v-model="scShow" position="bottom" closeable close-icon-position="top-right" :style="{ height: '80%' }">
            <div class="popup sc-popup">
                ...
                <van-row>
                    <scroll-view class='xuanze' catchtouchmove='true' scroll-y="true" style="  display: flex" style='padding:0'>
                        <view class='xuanze_wrap' wx:if="{{jibinselect}}">
                            <block wx:for="{{arr}}">
                                <view class='text_wrap7 text_kuang'>{{item.medicineSubjectName}}</view>
                                <!-- <image src='../../image/shanchu.png' bindtap='shanchuxuanze' data-index="{{index}}" data-text="{{item.name}}"></image>   -->
                            </block>
                        </view>
                        <view class='hist1'>*将按您的选择顺序依次展示在您的主页</view>
                        <!-- <view class='hist1'>*选择多个擅长后可拖动调整顺序</view> -->
                        <block wx:for="{{beGood}}">
                            <view class='section'>
                                <!-- -->
                                <view class='section1' data-text="{{beGoodNametext}}">{{item.name}}</view>
                                <view class=" text_wrap7 {{item.check?'active1':''}}" data-index="{{index}}" wx:for="{{item.list}}" wx:if="{{arrid==item.beGoodName?'item.check=true':'item.check=true'}}" data-check="{{item.check}}" data-text="{{item.beGoodName}}" id="{{item.beGoodId}}" bindtap='selectjib' data-index="{{index}}" data-find="{{item.find}}">{{item.beGoodName}}</view>
                            </view>
                        </block>
                        <view style='width:100%;height:72rpx;'></view>
                        <view class='generate1' bindtap='skilled'>
                            <view>保存</view>
                        </view>
                    </scroll-view>
                </van-row>
...
export default {
    ...
    methods: {
        ...
        ksShowData() {
            ...
            this.axios(home.home + '/mkkMoblie/doctorDataList/sectionList', {
                params: {
                    token: token
                }
            }).then(res => {
                this.shanchangt = res.data.data
                this.shanchang = true
                this.hidden = false
