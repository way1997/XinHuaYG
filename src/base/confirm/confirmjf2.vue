    ...
    <div class="confirm" v-show="showFlag" @click.stop>
        <div class="confirm-wrapper">
            <div class="confirm-content">
                <p class="text">
                    <span style="font-size:.23rem">请输入兑换金额 （1积分=1元）</span>
                    <van-field v-model="value" label="¥" placeholder="请输入金额" />
                </p>
                <p class="text-tit">
                    您的积分余额为 {{text}} , <span @click="allMoney">全部兑换</span>
                </p>
                <div class="operate">
        ...
        confirm() {
            ...
            if (isNaN(this.value)) {
                ...
                if (!this.value) {
                    ...
                    if (valMoney == NaN) {
                        Toast.fail('您输入有误，无法兑换');
                    }
                    if (valMoney <= 0) {
                        Toast.fail('您输入金额有误，无法兑换');
                    } else if (valMoney > celMoney) {
                        Toast.fail('您输入金额超过剩余积分总额，无法兑换');
                    } else {
