...
//待问诊列表删除
export function delChatNum(list) {
  const url = baseURI + '/mkkMoblie/chat/delChatNum'
...
// 职称业务开启
export function zcSelc(list) {
  const url = baseURI + '/mkkMoblie/doctorDataList/titleList'
  return new Promise((resolve, reject) => {
    axios.post(url, qs.stringify({
      token: list.token
    })).then((res) => {
...
// 科室业务Data获取
export function ksShowData(list) {
...
//用户信息获取
export function getUser(list) {
  const url = baseURI + '/mkkMoblie/doctorDataList/beGoodList?token=' + list.token
  return new Promise((resolve, reject) => {
    axios.post(url, qs.stringify({
      token: list.token
    })).then((res) => {
...
//用户地区获取
export function getAreaUser(list) {
  const url = baseURI + '/mkkMoblie/doctorSupplyThree/getDoctorInfo?doctorId=' + list.doctorId + "&token=" + list.token
  return new Promise((resolve, reject) => {
    axios.post(url, qs.stringify({
      doctorId: list.doctorId,
      token: list.token
...
//认证下一步提交
export function zenzhengSubmit(list) {
  const url = baseURI + '/mkkMoblie/doctorSupplyThree/saveDoctorInfo'
  return new Promise((resolve, reject) => {
    axios.post(url, qs.stringify({
      doctorId: list.doctorId,
      token: list.token,
      ...
      doctorPhoto: list.doctorPhoto,
      areaId: list.areaId,
      doctorHospitalId:list.doctorHospitalId,
      doctorSex: list.doctorSex,
      jobLevelId: list.jobLevelId,
      doctorDepartmentId: list.doctorDepartmentId,
      subjectIds: list.subjectIds,
      remarks: list.remarks,
      idCard: list.idCard,
      certificateCoding: list.certificateCoding
    })).then((res) => {
      resolve(res.data)
    })
  })
...
//认证提交并审核
export function rzSubmitShenghe(list){
  const url = baseURI + '/mkkMoblie/doctorSupplyThree/saveCertification'
  return new Promise((resolve, reject) => {
    axios.post(url,qs.stringify({
      doctorId: list.doctorId,
      token: list.token
...
//证书认证首屏加载
export function zsrzLoad(list){
  const url = baseURI + '/mkkMoblie/doctorSupplyThree/getCertification'
  return new Promise((resolve, reject) => {
    axios.post(url,qs.stringify({
      doctorId: list.doctorId,
      token: list.token
...
//证书认证提交审核
export function generateData(list){
  const url = baseURI + '/mkkMoblie/doctorSupplyThree/saveCertification'
  return new Promise((resolve, reject) => {
    axios.post(url,qs.stringify({
      doctorId: list.doctorId,
